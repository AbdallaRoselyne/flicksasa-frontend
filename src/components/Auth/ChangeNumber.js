...
const ChangeNumber = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      setError(error.response?.data?.message || 'Failed to change phone number. Please try again.');
    }
  };
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-cover bg-center" 
      ...
      <div className="bg-black bg-opacity-70 p-8 rounded-lg max-w-md w-full space-y-8">
        <h2 className="text-lg sm:text-xl font-extrabold text-center text-white mb-4">Change Phone Number</h2>
        {error && (
