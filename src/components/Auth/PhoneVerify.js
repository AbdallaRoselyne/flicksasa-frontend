...
const PhoneVerify = () => {
  ...
  useEffect(() => {
    if (locationState.state?.phoneNumber) {
      ...
      setMessage(`A verification code has been sent to ${locationState.state.phoneNumber} (Safaricom number only). Please check your messages.`);
    } else {
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await api.post('/api/auth/resend-verification-code', { phoneNumber });  // Updated to send an object
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-cover bg-center" 
      ...
      <div className="bg-black bg-opacity-70 p-8 rounded-lg max-w-md w-full space-y-8">
        <div className="text-center">
          <h2 className="text-lg sm:text-2xl font-extrabold text-white mb-2">Phone Number Verification</h2>
